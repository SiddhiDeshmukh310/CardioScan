...
for ecg_id, row in clean_df.iterrows():
    ...
    try:
        ...
        # Save sample records for app (up to 12 records, 4 of each class)
        if len(sample_records) < 12:
            ...
            if lbl_count < 4:
                ...
                np.save(sample_file, signal) # raw signal for plotting/demo
...
print(f"Split counts -> Train (1-8): {len(X_train)}, Val (9): {len(X_val)}, Test (10): {len(X_test)}")
...
# Save sample metadata index for app
...
print(f"Waveform Test Accuracy: {test_acc:.4f}")
print(f"Waveform Test Macro F1: {test_macro_f1:.4f}")
print(f"Waveform Test Macro AUROC: {test_auroc:.4f}")
