...
for idx, row in df.iterrows():
    ...
    for ext in ['.dat', '.hea']:
        local_file = os.path.normpath(os.path.join('data', 'ptbxl_100hz', rel_path + ext))
        if not os.path.exists(local_file):
            remote_url = base_url + rel_path + ext
            files_to_download.append((remote_url, local_file))

print(f'Total files needed: {len(df)*2}, Files remaining to download: {len(files_to_download)}')
...
def download_file(item):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=20) as res, open(local_file, 'wb') as f:
            f.write(res.read())
        return True
...
if files_to_download:
    print(f'Starting multi-threaded download of {len(files_to_download)} files with 100 workers...')
    completed, failed = 0, 0
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = {executor.submit(download_file, item): item for item in files_to_download}
        for future in as_completed(futures):
            res = future.result()
            if res is True: completed += 1
            else: failed += 1
            if (completed + failed) % 2000 == 0 or (completed + failed) == len(files_to_download):
